...
// FOAM (on the surface, carried by the fluid), BUBBLES (submerged,
// buoyant, dragged toward the fluid velocity).
...
void main()
{
    ...
    for (int dz = -1; dz <= 1; ++dz)
        for (int dy = -1; dy <= 1; ++dy)
            for (int dx = -1; dx <= 1; ++dx) {
                ...
                int guard = 0;
                while (j >= 0 && guard++ < 64) {
                    if (p[j].posLife.w > 0.0 && length(xi - p[j].posLife.xyz) < u_h) {
                        ++neighbors;
                        fluidVel += p[j].vel.xyz;
                    }
                    j = nxt[j];
                }
            }
    if (neighbors > 0) fluidVel /= float(neighbors);
    ...
    else if (neighbors < 22) {
        type = 1.0; // FOAM: ride the surface
        vi = mix(vi, fluidVel, 0.85);
        lifeDecay = 1.0;
    }
    else {
        type = 2.0; // BUBBLE: buoyancy + drag toward fluid motion
        vi += -u_gravity * 2.2 * u_dt;
        vi = mix(vi, fluidVel, 0.6);
        lifeDecay = 1.3;
    }

    xi += vi * u_dt;
    if (xi.y < 0.005) { xi.y = 0.005; vi.y = abs(vi.y) * 0.2; } // ground
    xi = clamp(xi, u_domainMin, u_domainMax);

    d[i].posLife = vec4(xi, d[i].posLife.w - lifeDecay * u_dt);
