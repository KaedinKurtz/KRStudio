...
void RenderingSystem::initializeResourcesForContext(QOpenGLFunctions_4_3_Core* gl, Scene* scene) {
    ...
    try {
        ...
        contextShaders["pointcloud"] = Shader::build(gl, (shaderDir + "pointcloud_vert.glsl").toStdString(), (shaderDir + "pointcloud_frag.glsl").toStdString()).release();
    }
    ...
    if (m_renderPasses.size() == 0) {
        ...
        auto pointCloudPass = std::make_unique<PointCloudPass>();
        m_pointCloudPass = pointCloudPass.get(); // Get the raw pointer before moving ownership
        m_renderPasses.push_back(std::move(pointCloudPass));
    }
