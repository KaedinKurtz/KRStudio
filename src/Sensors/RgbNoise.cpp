...
double RgbNoiseModel::apply(double signalDN, std::mt19937_64& rng) const {
    ...
    if (signalDependent) {
        // shot noise: electrons e = S * photonsPerDN ~ Poisson(e); back to DN gives variance = S/photonsPerDN.
        const double e = S * photonsPerDN;
        double shotDN;
        if (e < 1e-9) {
            shotDN = 0.0;
        } else {
            std::poisson_distribution<long long> pois(e);
            shotDN = double(pois(rng)) / photonsPerDN;
        }
